...
import org.snomed.otf.reasoner.server.service.data.DataType;
...
import static org.snomed.otf.reasoner.server.service.model.SnomedOntologyUtils.*;
...
public final class ConcreteDomainDefinition implements Definition, Serializable {
	 ...
	 * @param unitId the concept identifier of the UOM concept, or NOT_APPLICABLE if no unit is set for this definition
	...
	@Override public String toString() {
		return "ConcreteDomainDefinition [label=" +
				label +
				", unitId=" +
				unitId +
				", literal=" +
	...
	private String sanitize(String text) {
		...
		for (int i = 0; i < FIND.length; i++) {
			text = StringUtils.replace(text, FIND[i], REPLACE[i]);
		}
		return text;
	}
